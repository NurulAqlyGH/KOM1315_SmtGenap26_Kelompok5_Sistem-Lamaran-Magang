...
import { loginUser, setAuthToken, setAuthRole, clearAuth } from '../../services/api';
...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (data.role === 'mahasiswa') navigate('/mahasiswa/dashboard');
      else if (data.role === 'dosen') navigate('/dosen/dashboard');
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6">
      ...
      <div className="bg-white border border-gray-100 rounded-3xl p-8 max-w-md w-full shadow-sm">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            <label className="text-sm font-semibold text-gray-700 block mb-1">Username, Email, NIM, atau NIP</label>
            <div className="relative">
              ...
              <input
                type="text"
                required
                placeholder="Masukkan username, email, NIM, atau NIP"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 bg-indigo-500 text-white font-semibold rounded-xl hover:bg-indigo-600 transition-colors shadow-md shadow-indigo-100 mt-2 disabled:opacity-50"
          >
